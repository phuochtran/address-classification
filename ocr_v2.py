...
class Solution:
    ...
    def process(self, input: str) -> dict:
        ...
        candidates = []
        for i in reversed(range(n)):  # start from the last token
            for j in reversed(range(max(0, i-max_window+1), i+1)):  # window forward (increasing size)
                substr = ' '.join(tokens[j:i+1])
                candidates.append((j, i+1, substr))
            ...
            match level:
                case "province":
                    ...
                    if results:
                        ...
                        if score+0.5 > output["province"]["score"] or (score+0.5== output["province"]["score"] and len(norm) > len(output["province"]["norm"])):
                            orig = self.map_province.get(norm, "")
                            output["province"] = {'orig': orig, 'norm': norm, 'score': score+0.5}
                ...
                case "district":
                    ...
                    if results:
                        for res in sorted(results, key=lambda x: x[1]):
                            ...
                            score = 1.0 - dist / max(len(substr), len(norm))
                            if score+0.5 > output["district"]["score"] or (score+0.5 == output["district"]["score"] and len(norm) > len(output["district"]["norm"])):
                                orig = self.map_district.get(norm, "")
                                output["district"] = {'orig':orig,'norm':norm,'score':score+0.5}
                ...
                case "ward":
                    ...
                    if results:
                        for res in sorted(results, key=lambda x: x[1]):
                            norm, dist = res
                            if output["province"]["orig"] and all(norm not in wards for wards in self.ref[output["province"]["norm"]].values()): continue
                            score = 1.0 - dist / max(len(substr), len(norm))
                            if score+0.5 > output["ward"]["score"] or (score+0.5 == output["ward"]["score"] and len(norm) > len(output["ward"]["norm"])):
                                orig = self.map_ward.get(norm, "")
                                output["ward"] = {'orig':orig,'norm':norm,'score':score+0.5}
                                break
                    print(f"output: {output}\n")
                case _:
                    # province
