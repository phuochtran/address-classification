    ...
    def _worker(self, substr: str) -> bool:
        ...
        match level:
            case "province":
                ...
                print(f"detected level: {level} - r: {results}")
                if results:
            ...
            case "district":
                ...
                if results:
                    norm, num, score = max(results, key=lambda x: (x[2], x[1]))
                    print(f"best matched - norm: {norm}, num: {num}, score: {score}")
                    if self.output["province"]["orig"] and norm not in self.ref[self.output["province"]["norm"]]: return False
        ...
        print(f"linear-scanned level: province - r: {results}")
        if results:
            norm, num, score = max(results, key=lambda x: (x[2], x[1]))
            print(f"best matched - norm: {norm}, num: {num}, score: {score}")
            if score+bias > self.output["province"]["score"] or (score+bias == self.output["province"]["score"] and num > self.output["province"]["num"]):
        ...
        print(f"linear-scanned level: district - r: {results}")
        if results:
            norm, num, score = max(results, key=lambda x: (x[2], x[1]))
            print(f"best matched - norm: {norm}, num: {num}, score: {score}")
            if self.output["province"]["orig"] and norm not in self.ref[self.output["province"]["norm"]]: return False
        ...
        print(f"linear-scanned level: ward - r: {results}")
        if results:
            norm, num, score = max(results, key=lambda x: (x[2], x[1]))
            print(f"best matched - norm: {norm}, num: {num}, score: {score}")
            if self.output["province"]["orig"]:
...
if __name__ == "__main__":
    sol = Solution()
    print(sol.process("xA hoa TinH, huYEN MaNg thiT, Tinh viN Long"))
#OA: 54.44%
